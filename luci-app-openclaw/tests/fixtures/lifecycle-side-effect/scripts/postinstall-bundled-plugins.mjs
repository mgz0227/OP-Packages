...
if (process.env.npm_config_ignore_scripts !== 'true') {
  console.error('[postinstall] 错误: npm_config_ignore_scripts 必须为 "true", 实际为: ' + process.env.npm_config_ignore_scripts);
  process.exit(98);
}
...
if (!fs.existsSync(localPkgPath)) {
  console.error('[postinstall] 错误: cwd 必须为安装包根目录，当前未找到 package.json: ' + cwd);
  process.exit(97);
}
...
try {
  ...
  console.error('[postinstall] 依赖解析失败: ' + err.message);
  process.exit(96);
}
...
if (process.env.OC_FAIL_POSTINSTALL === '1') {
  console.error('[postinstall] 模拟 postinstall 失败 (OC_FAIL_POSTINSTALL=1)');
  process.exit(43);
}
